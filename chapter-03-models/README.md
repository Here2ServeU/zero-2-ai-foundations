...
> Matches **Chapter 03** of the Foundations course video. Two real detectors, two different ways of learning.
...
Both programs plant the same "seed" for their random numbers, so you should see exactly these
numbers too. A perfect score here is not luck: the pretend data follows one clean rule (big
amounts in the middle of the night), and the model finds it easily. Real data is never this
tidy — that's why the report card below matters so much once the rule isn't obvious.
