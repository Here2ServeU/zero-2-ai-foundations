...
> Matches **Chapter 06** of the Foundations course video. The runnable scripts for this chapter are in this folder.
...
- **On a Mac, window 5000 is busy with nothing running** → macOS uses it for **AirPlay
  Receiver**. Turn it off in **System Settings → General → AirDrop & Handoff → AirPlay
  Receiver**, or map a different window on your side: `-p 5001:5000` in Step 4 (then knock on
...
🎉 **That's the foundations course.** You started with an empty computer and finished with a
machine learning model that other programs can call, packed in a container that runs anywhere.

Everything after this is about making that container survive the real world: real cloud
infrastructure, a pipeline that ships it for you, many copies running at once, monitoring that
watches it, and retraining when the world changes.

➡ [See what's next](../WHATS-NEXT.md) — the full 18-chapter *Zero to AI Systems Engineer*
course picks up right here, plus the two graduation systems (NexaGuard and ClarityAI).
