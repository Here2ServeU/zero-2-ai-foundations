...
> Matches **Chapter 05** of the Foundations course video. The runnable scripts for this chapter are in this folder.
...
```
{"label":"FRAUD","confidence":1.0,"prediction":1,"req_id":"a1b2c3d4",...}
```
...
- **On a Mac, port 5000 is busy even though you started nothing** → macOS itself uses window
  5000 for **AirPlay Receiver**. Turn it off: **System Settings → General → AirDrop & Handoff →
  AirPlay Receiver → Off**. Then run Step 3 again. (If you'd rather leave AirPlay on, change the
  last line of `app/api.py` to `port=5001` and use `5001` in every `curl` command below.)
- **Windows: `venv\Scripts\activate` says "running scripts is disabled on this system"** →
  PowerShell blocks script files until you allow them. Run this once, answer **Y**, then try
  activating again:
